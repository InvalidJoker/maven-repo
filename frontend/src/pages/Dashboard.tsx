  ...
  return (
    <div>
      ...
      {repos && repos.length === 0 && (
        <Card className="p-6 text-sm text-slate-500">No repositories available yet.</Card>
      )}
          ...
          >
            <Card className="flex items-center justify-between p-4 transition-colors hover:border-slate-700 hover:bg-slate-900">
              <div className="min-w-0">
                ...
                <code className="mt-1 block truncate text-xs text-slate-500">{repoUrl(repo.name)}</code>
              </div>
      ...
      <Card className="mt-8 p-5">
        <h2 className="mb-3 text-sm font-semibold text-slate-200">Using a repository in your build</h2>
        <InstallSnippet
        ...
        <p className="mt-3 text-xs text-slate-500">
          ...
          <span className="text-slate-300">Tokens</span> to publish or read private repositories.
        </p>
      </Card>
    </div>
  )
